...
class SnapshotManager {
     ...
     * @returns {Bluebird}
     */
    create(name = 'latest', devSnapshot = true) {
     ...
     * @returns {Bluebird}
     */
    remove(name = 'latest', devSnapshot = true) {
        let fileLocation = this.fileLocation(name, devSnapshot);
        return new Bluebird((resolve, reject) => {
     ...
     * @returns {Bluebird}
     */
    fetch(name = 'latest', devSnapshot = true) {
        let fileLocation = this.fileLocation(name, devSnapshot);
        return new Bluebird((resolve, reject) => {
    ...
    diff(oldMapping, newMapping) {
        ...
        diff.remain.forEach(entity => {
            ...
            let previous = index_1.Mapping.restore(oldMapping[entity]);
            let current = index_1.Mapping.restore(newMapping[entity]);
            ...
            fieldDiff.create.forEach((property) => {
            ...
            relationDiff.drop.forEach(property => {
                ...
                if ((relation.type === index_1.Mapping.RELATION_MANY_TO_ONE) || (relation.type === index_1.Mapping.RELATION_ONE_TO_ONE && !relation.mappedBy)) {
                    // We own the fk.
                ...
                if (relation.type === index_1.Mapping.RELATION_ONE_TO_MANY || !relation.inversedBy) {
                    return;
                }
            ...
            relationDiff.remain.forEach(property => {
                ...
                if ((newRelation.type === index_1.Mapping.RELATION_MANY_TO_ONE) || (newRelation.type === index_1.Mapping.RELATION_ONE_TO_ONE && !newRelation.mappedBy)) {
                    return;
                }
                // Nothing to do for other side.
        ...
        function createRelation(mapping, property, targetMapping, create) {
            ...
            if ((relation.type === index_1.Mapping.RELATION_MANY_TO_ONE) || (relation.type === index_1.Mapping.RELATION_ONE_TO_ONE && !relation.mappedBy)) {
                ...
                if (!targetMapping.columns[joinColumn.referencedColumnName]) {
                    throw new Error(`Cannot create foreign key for '${tableName}', ` +
            ...
            buildJoinTable(mapping.fields[property].joinTable, tableName, targetMapping.entity.tableName, instructions.create, instructions.foreign);
        }
     ...
     * @returns {{}}}
     */
    postDiffingOperations(instructions) {
...
SnapshotManager.SNAPSHOTS_PATH = 'snapshots';
SnapshotManager.DEV_SNAPSHOTS_PATH = 'dev_snapshots';
