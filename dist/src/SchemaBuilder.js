...
class SchemaBuilder {
    ...
    runCode() {
        let migration = {
            getBuilder: store => {
                let connection = this.entityManager.getStore(store).getConnection(Store_1.Store.ROLE_MASTER);
     ...
     * @returns {SchemaBuilder}
     */
    process(instructionSets) {
        let spaceCount = 4;
        let allCode = [];
        let spacing = (change = 0) => {
            spaceCount += change;
     ...
     */
    buildDropForeignKeys(drop, code, spacing) {
     ...
     * @param {function}  spacing
     */
    buildCreateForeignKeys(create, code, spacing) {
     ...
     * @param {function}  spacing
     */
    buildTable(action, createForeign, instructions, code, spacing) {
     ...
     * @param {boolean}      alter
     */
    composeField(field, alter = false) {
        let code = 'table';
