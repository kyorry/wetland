...
function getUnitOfWork(entities, config = {}) {
    let wetland = new Wetland_1.Wetland(config);
    if (entities) {
...
describe('UnitOfWork', () => {
    ...
    describe('.registerCollectionChange()', () => {
        ...
        it('should register a new relation for a collection', () => {
            ...
            chai_1.assert.lengthOf(relations.added.simples, 2);
            chai_1.assert.lengthOf(Object.keys(relations.added), 1);
            chai_1.assert.lengthOf(Object.keys(relations.removed), 0);
        });
        it('should register a removed relation for a collection', () => {
            ...
            chai_1.assert.lengthOf(relations.removed.simples, 1);
            chai_1.assert.lengthOf(Object.keys(relations.removed), 1);
            chai_1.assert.lengthOf(Object.keys(relations.added), 0);
        });
        it('should clean up changed properties and reference on revert', () => {
            ...
            let relations = MetaData_1.MetaData.forInstance(parent).fetch('entityState.relations');
            chai_1.assert.strictEqual(relations.added.simples[0], simple1);
            chai_1.assert.lengthOf(relations.added.simples, 1);
            chai_1.assert.lengthOf(Object.keys(relations.added), 2);
            chai_1.assert.lengthOf(Object.keys(relations.removed), 0);
    ...
    describe('.prepareCascades()', () => {
        ...
        it('should prepare no cascades when not needed', () => {
            ...
            chai_1.assert.lengthOf(unitOfWork.getDeletedObjects(), 0);
            chai_1.assert.lengthOf(unitOfWork.getDirtyObjects(), 0);
            chai_1.assert.lengthOf(unitOfWork.getNewObjects(), 0);
            chai_1.assert.lengthOf(unitOfWork.getCleanObjects(), 0);
            chai_1.assert.lengthOf(unitOfWork.getRelationshipsChangedObjects(), 0);
        });
        it('should prepare cascades for one-sided relationship', function () {
            let config = { mapping: { defaults: { cascades: ['persist'] } } };
            let unitOfWork = getUnitOfWork([Simple_1.Simple, Parent_1.Parent], config);
            let entityManager = unitOfWork.getEntityManager();
            let parent = new Parent_1.Parent;
            let simple1 = new Simple_1.Simple;
            let simple2 = new Simple_1.Simple;
            let simple3 = new Simple_1.Simple;
            ...
            chai_1.assert.lengthOf(unitOfWork.getDeletedObjects(), 0);
            chai_1.assert.lengthOf(unitOfWork.getDirtyObjects(), 0);
            chai_1.assert.lengthOf(unitOfWork.getNewObjects(), 4);
            chai_1.assert.lengthOf(unitOfWork.getCleanObjects(), 0);
            chai_1.assert.lengthOf(unitOfWork.getRelationshipsChangedObjects(), 1);
            chai_1.assert.strictEqual(unitOfWork.getRelationshipsChangedObjects()[0].single, simple1);
            chai_1.assert.sameMembers(unitOfWork.getRelationshipsChangedObjects()[0].simples, [simple2, simple3]);
        });
        it('should prepare cascades for persist when new relation', () => {
            ...
            chai_1.assert.lengthOf(unitOfWork.getDeletedObjects(), 0);
            chai_1.assert.lengthOf(unitOfWork.getDirtyObjects(), 0);
            chai_1.assert.lengthOf(unitOfWork.getNewObjects(), 3);
            chai_1.assert.lengthOf(unitOfWork.getCleanObjects(), 0);
            chai_1.assert.lengthOf(unitOfWork.getRelationshipsChangedObjects(), 1);
        ...
        it('should throw an error for persist when new relation has no cascade persist set', () => {
            // Fresh start.
            MetaData_1.MetaData.clear(product_1.Product, user_1.User);
        ...
        it('should prepare cascades for persist when new collection', () => {
            ...
            unitOfWork.prepareCascades();
            chai_1.assert.lengthOf(unitOfWork.getNewObjects(), 2);
        });
        it('should prepare cascades recursively (aka the shit-show test)', () => {
            ...
            entityManager.persist(product);
            chai_1.assert.lengthOf(unitOfWork.getNewObjects(), 1);
            chai_1.assert.lengthOf(unitOfWork.getRelationshipsChangedObjects(), 1);
            unitOfWork.prepareCascades();
            chai_1.assert.lengthOf(unitOfWork.getDeletedObjects(), 0);
            chai_1.assert.lengthOf(unitOfWork.getDirtyObjects(), 0);
            chai_1.assert.lengthOf(unitOfWork.getNewObjects(), 11);
            chai_1.assert.lengthOf(unitOfWork.getCleanObjects(), 3);
            // 1 product, 1 category (other two don't have relations), 3 tags, 1 image
            chai_1.assert.lengthOf(unitOfWork.getRelationshipsChangedObjects(), 7);
            chai_1.assert.deepEqual(MetaData_1.MetaData.forInstance(product).fetch('entityState.relations'), {
                removed: {},
                added: {
        ...
        it('should persist all the changes properly (aka the shit-show-but-with-persist-as-well test)', done => {
            ...
            wetland.getSchemaManager().create().then(() => {
                ...
                entityManager.flush().then(() => {
                    entityManager.getRepository(user_1.User).findOne({ name: 'test tag one creator' }, { populate: { 'profile': 'p' } })
                        .then(user => {
    ...
    describe('.processAfterCommit()', () => {
        it('Should call post-lifecyle callbacks hook with a clean unit of work', callback => {
            ...
            let wetland = new Wetland_1.Wetland({ entities: [User] });
